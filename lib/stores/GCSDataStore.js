...
class GCSDataStore extends DataStore {
    ...
    create(req) {
        return new Promise((resolve, reject) => {
            ...
            const write_stream = gcs_file.createWriteStream(options);
            const read_stream = new stream.PassThrough();
            read_stream.on('error', reject);
            write_stream.on('error', reject);

            // Add the upload id to the url
            write_stream.on('response', (res) => {
                if (res.headers['x-guploader-uploadid'] !== undefined) {
                    file.id += `&upload_id=${res.headers['x-guploader-uploadid']}`;
                }
                resolve(file);
            });
    ...
    write(req, file_id, offset) {
        ...
        .then((data) => {
            return new Promise((resolve, reject) => {
                const file = this.bucket.file(file_id.split('&upload_id=')[0]);
                const options = {
                    offset,
                ...
                write_stream.on('response', (res) => {
                    console.log(`[GCSDataStore] write: Write stream complete (${res.statusCode}): ${new_offset} bytes written to GCS`);
                    resolve(new_offset);
                });

                write_stream.on('error', (e) => {
                    console.log('[GCSDataStore] write: Write stream error', e);
                    reject(ERRORS.FILE_WRITE_ERROR);
                });
